// js/comment-manager-admin.js
// =====================================================
// Comment Manager — Admin panel
// Loads comments from both announcements and
// communityPosts subcollections for a given barangay.
//
// Architecture:
//   1. Fetch recent posts from both collections (up to 50 each)
//   2. For each post, fetch its top-level comments
//   3. Replies loaded on demand (expand button)
//   4. Delete works on both comments and replies
//   5. Source filter: all / announcements / communityPosts
//   6. Search filter: author name or comment body
// =====================================================
...
let _bid          = null;
let _barangay     = null;
let _allComments  = []; // flat list of { commentId, postId, postTitle, col, ...commentData }
...
async function loadAllComments() {
  ...
  for (const src of sources) {
    ...
    for (const postDoc of postsSnap.docs) {
      ...
      // Build a recursive tree from flat comment list
        const commentMap = new Map(
        commentsSnap.docs.map(d => ({ id: d.id, ...d.data(), replies: [] }))
            .map(c => [c.id, c])
        );
        ...
        node.replies.sort((a, b) => (a.createdAt?.toDate?.() ?? 0) - (b.createdAt?.toDate?.() ?? 0));
  ...
  return replies.map(r => {
      ...
      <div style="margin-top:.35rem;">
        <button
          ...
          style="background:none;border:none;cursor:pointer;font-size:.72rem;
            color:#6b7280;font-weight:600;padding:0;display:flex;
            align-items:center;gap:.3rem;transition:color .15s;"
    ...
    return `
      <div style="display:flex;gap:.6rem;align-items:flex-start;
        padding:.5rem .6rem;margin-left:${indent}px;
        border-left:2px solid #e5e7eb;margin-top:.35rem;">
        <div style="width:20px;height:20px;border-radius:50%;background:#e5e7eb;
          display:flex;align-items:center;justify-content:center;
          font-size:.55rem;font-weight:700;color:#6b7280;flex-shrink:0;">
        ...
        <div style="flex:1;min-width:0;">
          <div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;margin-bottom:2px;">
            <span style="font-size:.78rem;font-weight:700;color:#374151;">${esc(r.authorName ?? 'Resident')}</span>
            <span style="font-size:.68rem;color:#9ca3af;">${rTime}</span>
            <span style="font-size:.68rem;color:#9ca3af;">❤️ ${r.likeCount ?? 0}</span>
          </div>
          <p style="font-size:.8rem;color:#4b5563;margin:0;line-height:1.4;word-break:break-word;">
            ${esc(r.body ?? '')}
          </p>
          ${nestedSection}
        </div>
        <button
          ...
          style="flex-shrink:0;background:none;border:none;cursor:pointer;
            color:#d1d5db;padding:2px;transition:color .15s;"
...
function buildCommentRow(c) {
  ...
  const sourceChip = c.col === 'announcements'
    ? `<span style="background:#f0fdf4;color:#15803d;padding:2px 8px;border-radius:999px;
        font-size:.68rem;font-weight:700;border:1px solid #bbf7d0;">✓ Official</span>`
    : `<span style="background:#f9fafb;color:#6b7280;padding:2px 8px;border-radius:999px;
        font-size:.68rem;font-weight:700;border:1px solid #e5e7eb;">Community</span>`;
  ...
  return `
    <div style="background:#fff;border-radius:12px;padding:1.1rem 1.25rem;
      box-shadow:0 1px 4px rgba(0,0,0,.07);"
      id="comment-row-${esc(c.commentId)}">

      <!-- Header: post title + source chip -->
      ...
      <div style="display:flex;gap:.6rem;align-items:flex-start;">
        ...
        <!-- Avatar -->
        <div style="width:28px;height:28px;border-radius:50%;background:#e5e7eb;
          display:flex;align-items:center;justify-content:center;
          font-size:.62rem;font-weight:700;color:#6b7280;flex-shrink:0;">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <p style="font-size:.85rem;color:#4b5563;margin:0 0 .3rem;
        ...
        <button
          ...
          style="flex-shrink:0;display:inline-flex;align-items:center;gap:.3rem;
            padding:.4rem .75rem;border-radius:7px;border:1.5px solid #fca5a5;
            background:#fff;color:#dc2626;font-size:.75rem;font-weight:600;
...
// ── Delete a comment (and update commentCount on post) ─────────────
window.adminDeleteComment = async function(postId, commentId, col) {
  ...
  try {
    ...
    function removeFromTree(replies, commentId) {
    return replies
        .filter(r => r.id !== commentId)
        .map(r => ({ ...r, replies: removeFromTree(r.replies ?? [], commentId) }));
    }
    ...
    .map(c => ({
        ...
        replyCount: removeFromTree(c.replies ?? [], commentId).length,
    }));
    // Also remove if it was a reply stored inside a parent
    _allComments = _allComments.map(c => ({
      ...c,
...
// Auto-load when the comments sub-tab is opened
