...
onAuthStateChanged(auth, async (user) => {
  ...
  onSnapshot(q, snap => {
    ...
    if (badge) { badge.textContent = reports.length; badge.style.display = reports.length ? 'inline' : 'none'; }
...
function renderReports(reports) {
  ...
  if (!el) return;
  if (!reports.length) {
    el.innerHTML = `<div style="background:#fff;border-radius:12px;padding:3rem;text-align:center;color:#aaa;box-shadow:0 1px 4px rgba(0,0,0,.07);">
      <div style="font-size:2rem;margin-bottom:.5rem;">🛡️</div>
      <p style="margin:0;font-size:.9rem;">No reported posts.</p></div>`;
    return;
  }
  el.innerHTML = reports.map(r => `
    <div style="background:#fff;border-radius:12px;padding:1.25rem;
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap;">
        <div>
          <p style="font-weight:700;font-size:.9rem;margin:0 0 .25rem;">${esc(r.postTitle)}</p>
  ...
  try {
    ...
    for (const col of ['communityPosts', 'announcements']) {
      try {
        ...
        // Officers cannot delete admin posts
...
window.viewReportedPost = async function(postId) {
  if (!document.getElementById('reportPreviewModal')) {
    ...
    m.style.cssText = `display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);
  ...
  if (imgs.length) {
    ...
    // Grid of thumbnails — click opens fullscreen viewer
    ...
    // Single image: full width, not split into 50%
    if (imgs.length === 1) {
      grid.firstChild.style.flex = '1 1 100%';
    }
