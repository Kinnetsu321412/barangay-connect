...
let _editOriginalData = null;   // snapshot when edit started (for dirty-check)
...
// ── Normalize affects (handles legacy lowercase → Title Case) ──────
...
onAuthStateChanged(auth, async (user) => {
  ...
  renderForm(null); // Initially renders the "Add Schedule" button
});
...
// Updates only the two warning banners inside the edit form
...
function typeChip(type) {
  ...
  return `<span style="background:${t.bg};color:${t.color};padding:1px 7px;
...
function buildListRow(id, d, now) {
  ...
  // Start display:none — updateActiveBadges() controls visibility via style.display
  const liveBadge = `
  ...
  const endLabel = d.endTime < d.startTime
    ? `${d.endTime} <span style="font-size:.65rem;color:#aaa;">+1d</span>` : d.endTime;
      ...
      <div style="display:flex;gap:.35rem;align-items:center;flex-shrink:0;">
        ${!isExpired ? `
          ...
          <button onclick="curfewEdit('${id}')" title="${editTitle}"
            ...
            onmouseover="this.style.background='${editHover}'" onmouseout="this.style.background='${editBg}'">
  ...
  try {
    if (nowActive) {
      ...
      await setDoc(ref, {
        ...
        source: 'admin', active: true, dismissible: false,
        createdAt: serverTimestamp(), createdBy: auth.currentUser?.uid ?? 'system', expiresAt: null,
      });
    } else {
...
window.curfewDelete = async function(id, name) {
  const ok = await showConfirmModal({
    ...
    confirmLabel: 'Delete', iconName: 'trash-2', iconType: 'admin', danger: true,
  });
  if (!ok) return;
  try {
    const schedule = _schedules.find(s => s.id === id);
...
window.curfewEdit = async function(id) {
  ...
  // Clicking the Edit/Cancel button on the currently-editing row
  if (_editId === id) {
    if (_isFormDirty()) {
      const ok = await showConfirmModal({
        title: 'Discard Changes?',
        body:  `Unsaved edits to <strong>${esc(_editOriginalData?.name || 'this schedule')}</strong> will be lost.`,
        confirmLabel: 'Discard', iconName: 'alert-triangle', iconType: 'officer', danger: false,
      });
      if (!ok) return;
    }
...
window.curfewSave = async function() {
  ...
  // Affects — handle the Custom option
  ...
  const date = type === 'once' ? (document.getElementById('cfDate')?.value || null) : null;
...
window.curfewTypeChange = function() {
  ...
  document.querySelectorAll('input[name="cfType"]').forEach(inp => {
    const lbl = inp.closest('label'); const on = inp.value === type;
...
window.curfewAffectsChange = function() {
  ...
  document.querySelectorAll('input[name="cfAffects"]').forEach(inp => {
    const lbl = inp.closest('label'); const on = inp.value === val;
    lbl.style.background  = on ? '#1a3a1a' : '#fff';
    lbl.style.color       = on ? '#fff'    : '#555';
    lbl.style.borderColor = on ? '#1a3a1a' : '#e0e0e0';
  });
  ...
  el.innerHTML = `
    <div style="background:#fff;border-radius:12px;padding:1.5rem;
      ...
      <div style="display:grid;gap:1rem;">
        ...
        <!-- Enforcing warning — shown when schedule is live right now -->
